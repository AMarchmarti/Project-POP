...
![Diagrama de componentes](https://github.com/AMarchmarti/Project-POP/blob/master/imgDoc/diagComponentes.png)
...
Se ha usado Flexbox en casi todos los elementos de la web, facilitando la colocación de los elementos, ya que este sistema de cajas flexibles nos permite usar una serie de propiedades, tales como ``justify-content`` o ``align-content`` que nos permiten ajustar la colocación de los elementos de manera más natural, olvidando por otra parte ajustar la web de modo estática usando **paddings y margins**. 
También hemos optado por el uso de grid en los calendarios y en la gráfica de correlaciones, esto lo hemos elegido por varias razones, por una parte los requisitos del proyecto y en parte los requisitos del mismo elemento. Que queremos decir con esto? El elemento se estaba diseñando para su colocación como si fuera un calendario, entonces grid nos permitía dividir el elemento como queríamos.

Hemos intentado hacer una web dinámica, con esto queremos referirnos que gran parte del proyecto se crea mediante la manipulación del ``DOM``, esto quiere decir que los elementos html son creados dinámicamente mediante nuestra lógica. Los elementos estáticos aparecen en html natural, ya que estos elementos no provienen de ninguna variable, sin embargo, por ejemplo el calendario, lo hemos efectuado mediante sentencias ``js``, ya que debíamos tener tantos elementos ``<main>`` como el computo total de días del diario, dividido 30 (*días de un mes*). Siguiendo este mismo ejemplo entonces, la creación del modal variable según el dia que seleccionabamos, por tanto su comportamiento era dinámico y el contenido dependía del código. Igualemnte podrás comprovar que el ``css`` corresponde a cada clase, de la cuál hemos llamado.

El proyecto usa ``sass``, permitiendo una estructuración por componentes, simplificando su lectura. Cada uno toca el estilo de un componente y se juntan en un style.scss donde se importan. Se ha intentado usar siempre la encapsulación y se han usado variables y se ha creado una paleta de colores.

Queremos destacar la implementación de la **gráfica**, ya que sus estilo podrás comprobar mediante el uso del navegador que están incorporados en linia. Esto es debido a que su creación dinámica, con esto queremos decir que el tamaño de la barras y su color depende del contenido que le pasemos. Hemos recurrido a esto porque con los resultados negativos la barra de la gráfica no aparecía ya que la propiedad del ``css -> width`` no admite valores negativos. Entonces se nos ocurrió la idea de que esos valores se pasaban a positivos, creando así la barra, pero creando una paleta de colores negativos, pudiendo hacer un efecto visual muy acertado. Como resultado hemos obtenido una gráfica en forma de ``U`` que lleva de valores negativos a positivos degradando su color, colores más ``fríos`` para los **negativos** a los colores más ``cálidos`` que sería los **positivos**.

Queremos destacar que esta paleta la encontraremos tanto en el código js en el fichero ``graph.js``, como si observamos en el fichero ``styles.scss`` veremos la propiedad *root*, dónde encontramos la paleta de colores generales
...
SEguido de esto vino la parte de interficies, que llevo el computo total de ``35h`` (*seguro que se nos perdieron horas por el camino*), esta parte fue más dificil de calcular ya que al tocar simplemente los estilos podíamos hacerlo en cualquier momento, por tanto el seguimiento de este tiempo fue complicado. Aprecíamos este total ya que gran parte del tiempo estuvimos. El trabajo más laborioso fue la parte de las animaciones, al intentar que la web fuera moderna, agraciada para la vista y bastante sencilla, queríamos aplicar ciertas animaciones que dieran vida al proyecto, esto conllevo una suma de horas bastante alta.
